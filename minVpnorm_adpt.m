...
solids1 = find(T(:,5)==3);%find solid average elements
solids2 = find(T(:,5)==4);%find solid average elements
...
%solid average
loadedelem1 = solids1;
nloadedelem1 = size(loadedelem1,1);
%solid average
loadedelem2 = solids2;
nloadedelem2 = size(loadedelem2,1);
...
switch domain
    ...
    case 'biclamped'
        ...
        loadeddof1 = edofMat(loadedelem1,2:2:8);%load average
        loadeddof2 = edofMat(loadedelem2,2:2:8);%load average
    otherwise
        loadeddof = edofMat(loadedelem,2:2:8);
end
loadeddof = loadeddof(:);
loadeddof1 = loadeddof1(:);
loadeddof2 = loadeddof2(:);
loadmag = -14; 
loadmag1 = -28;
loadmag2 = -20;
load_per_dof = loadmag/nloadedelem/4;
load_per_dof1 = loadmag1/nloadedelem1/4;
load_per_dof2 = loadmag2/nloadedelem2/4;
F = sparse(loadeddof,1,load_per_dof,ndof,1);
F1=sparse(loadeddof1,1,load_per_dof1,ndof,1);
F2=sparse(loadeddof2,1,load_per_dof2,ndof,1);
F=F+F1+F2;
% Supports
switch domain
    case 'biclamped'
        ...
        [supnodes,~] = find(X(:,2)==0); % Find nodes with y==0
        supdofs2 = union(2*supnodes-1,2*supnodes);
...
x(T(:,5)==3) = 1-1e-6; % Solids1 
x(T(:,5)==4) = 1-1e-6; % Solids1   
beta = 4;
njumps = 4;
...
xmin(T(:,5)==3) = 1-1e-3;% Lower bound for solids
xmin(T(:,5)==4) = 1-1e-3;% Lower bound for solids
