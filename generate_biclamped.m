...
for e = 1:nelem
    ...
    T(e,1:4) = [n1 n2 n3 n4];
      % Assign voids
   %if (y_cent>0.1*sizey && y_cent<0.4*sizey && x_cent>sizex-130 &&x_cent<sizex-90); T(e,5:6) = [1 1]; end
    
   
    % if (y_cent>sizey-2 ); T(e,5:6) = [2 1]; end %控制路面上solid的区域
    %assign solids (for load average)
   if (y_cent>sizey-5); T(e,5:6) = [3 0]; end
   if (y_cent>sizey-5&& (x_cent>sizex-140 && x_cent<sizex-80)); T(e,5:6) = [4 0];end
    % Assign solids (for load)
   if (y_cent>sizey-5&& (x_cent>sizex-120 && x_cent<sizex-100)); T(e,5:6) = [2 0]; end  %if (x_cent>sizex-5&& (y_cent>sizey-30 && y_cent<sizey-20)); T(e,5:6) = [2 0]; end
